...
func buildExternalUserInfo(token *oauth2.Token, userInfo *social.BasicUserInfo, name string) *models.ExternalUserInfo {
	...
	if len(userInfo.Groups) > 0 {
		...
		if setting.AutoAssignOrg && setting.AutoAssignOrgId > 0 {
			orgID = int64(setting.AutoAssignOrgId)
			plog.Debug("The user has a group assignment and organization membership is auto-assigned",
				"role", userInfo.Role, "orgId", orgID)
		} else {
			orgID = int64(1)
			plog.Debug("The user has a group assignment and organization membership is not auto-assigned",
				"role", userInfo.Role, "orgId", orgID)
		}
